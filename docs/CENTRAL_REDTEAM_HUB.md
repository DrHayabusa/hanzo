...
The central facade exports 16 tools. The GUI command launcher uses GET /api/arsenal/catalog for all 90 existing /api/tools POST adapters; categories and fields are extracted from bundled source without execution. The optional MCP bridge adds GET /api/mcp/optional, POST /api/mcp/optional/<id>/probe and POST /api/mcp/optional/<id>/call. It reads private server configuration and requires exact allowlisting plus confirmation for calls. See OPTIONAL_MCP.md.
