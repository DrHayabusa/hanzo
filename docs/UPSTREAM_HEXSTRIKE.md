...
<img src="static/hanzo-emblem.svg" alt="Hanzo flat human shinobi and katana emblem" width="170"/>

# HANZO
### Authorized Red-Team Control Plane

Hanzo is this repository's original operator experience for HexStrike: a
scope-gated GUI, lab topology, centralized MCP gateway, verified integration
health, and local/hosted LLM selection. It preserves the upstream HexStrike
engine and attribution below.

[Run Hanzo](#hanzo-gui) • [Page guide](docs/PAGE_GUIDE.md) • [Lab deployment](docs/LAB_DEPLOYMENT.md) • [Validation](docs/VALIDATION_REPORT.md)
...
This project includes a local web interface for health monitoring, target analysis,
authorized smart scans, tool inventory, process activity, a centralized red-team
integration hub, and an AI copilot that supports Ollama, Groq, OpenRouter, or
another OpenAI-compatible API.

#### This Mac: fastest way to start

Double-click **`VAPT Agent.command`** in Finder. Leave its Terminal window open,
then visit `http://127.0.0.1:8888/`. The launcher starts both the Flask API and
the local Ollama/Qwen service. Press Control-C in that Terminal window to stop it.

From VS Code or Terminal, the equivalent command is:

```bash
cd "/Users/mohammedshahid/Documents/New project/VAPT-Agent"
./start_vapt_agent.sh
```

The local installation includes Qwen 3 1.7B and six Apple-silicon security tools:
Nuclei, Subfinder, HTTPX, Katana, FFUF, and Gobuster. Reinstall or update those
scanner binaries with `./scripts/install_security_tools.sh`.

The dashboard reports every executable HexStrike knows about. An offline label
means the scanner executable is not on this project's `tools/bin` path or the
system `PATH`; it does not mean Ollama is broken. Many entries are Linux-only,
commercial desktop products, or cloud tools that require credentials, so a
macOS laptop cannot safely make all 127 entries available.

#### How the pieces connect

```text
Browser GUI (127.0.0.1:8888)
  -> Central red-team hub + scope gate
       -> MCP gateway and optional specialist adapters
  -> Flask / HexStrike decision engine
       -> local scanner executables in tools/bin
       -> Ollama API (127.0.0.1:11434) -> qwen3:1.7b
```

Use **Assessment** to profile a target or run an explicitly authorized smart
scan. Use **Tool matrix** to see what HexStrike can currently execute. Use
**AI copilot** to explain results or commands; enable “Attach latest assessment”
to give the selected model the current result JSON. Browser-entered API keys are
kept only for that browser session and are not persisted. The model advises and
explains, while the Flask server is the component that actually invokes scanners.

Use **Integration hub** to see verified availability for HexStrike, CVE MCP,
Burp MCP, Claude-BugHunter, Strix, PentAGI, PentestGPT, CAI, Decepticon, and
Shannon. The hub distinguishes online, installed, configured, and offline states
and creates a non-executing scoped engagement plan. See
[`docs/CENTRAL_REDTEAM_HUB.md`](docs/CENTRAL_REDTEAM_HUB.md) for deployment,
MCP configuration, adapter setup, and validation.
The latest evidence-backed local result is recorded in
[`docs/VALIDATION_REPORT.md`](docs/VALIDATION_REPORT.md).

For the complete Windows IIS + Splunk exercise, clone the companion
[HELPAG VAPT Test Site](https://github.com/DrHayabusa/HELPAG-VAPT-Test-Site) and
follow [`docs/LAB_DEPLOYMENT.md`](docs/LAB_DEPLOYMENT.md).

```bash
# Recommended on macOS (Python 3.11)
python3.11 -m venv .venv311
.venv311/bin/pip install -r requirements-core.txt
./start_vapt_agent.sh
```

Open `http://127.0.0.1:8888`. The launcher binds to localhost by default. Set
`VAPT_HOST` and `VAPT_PORT` only when you intentionally need different values.
Keep the Terminal window running while using the GUI; closing it stops the local
server. On macOS, you can also double-click `VAPT Agent.command` in Finder.

Local AI API checks:

```bash
curl http://127.0.0.1:8888/api/llm/status
curl http://127.0.0.1:8888/api/redteam/integrations
curl -X POST http://127.0.0.1:8888/api/redteam/validate -H 'Content-Type: application/json' -d '{}'
curl -X POST http://127.0.0.1:8888/api/llm/chat \
  -H 'Content-Type: application/json' \
  -d '{"message":"Explain what Nuclei does."}'
```
